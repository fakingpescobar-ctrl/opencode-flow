"""
Плавающий индикатор статуса Telegram бота для opencode.

Прикреплён к окну Windows Terminal, правее TTS-оверлея.
Читает ~/.opencode-tts/telegram-status.json (пишет telegram-watch-daemon.py):
    Telegram загружается / Telegram готов / Отвечаю… / Telegram ошибка

Запуск: pythonw telegram_overlay.py
"""
...
kernel32.CreateMutexW(None, True, "Local\\OpenCodeTTS_TelegramOverlay")
if ctypes.get_last_error() == 183:
    sys.exit(0)

# Watchdog: если родительский процесс (opencode) умер — завершаемся
_PARENT_PID = None
...
_CRASH_LOG = Path.home() / ".opencode-tts" / "overlay_tg_crash.log"
...
STATUS_PATH = Path.home() / ".opencode-tts" / "telegram-status.json"
STALE_SEC = 30
POLL_MS = 100
...
STATES = {
    "loading":  ("● TG загружается…", "#7a5c00", "#ffffff"),
    "ready":    ("● TG готов ✓",       "#1f7a33", "#ffffff"),
    "replying": ("💬 TG отвечает…",    "#1f5fd0", "#ffffff"),
    "error":    ("● TG ошибка",        "#5a1f1f", "#dddddd"),
    "offline":  ("● TG offline",       "#3a3a3a", "#bbbbbb"),
}

W, H = 190, 30
MARGIN = 6
TASKBAR = 56
X_OFFSET = (W + 8) * 2  # правее whisper + tts оверлеев
...
def is_parent_alive():
    if not _PARENT_PID:
        return True
    h = kernel32.OpenProcess(0x400, False, _PARENT_PID)  # PROCESS_QUERY_INFORMATION
    if not h:
        return False
    kernel32.CloseHandle(h)
    return True
